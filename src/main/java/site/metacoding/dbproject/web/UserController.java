...
public class UserController {
    ...
    // http://localhost:8000/api/user/username/same-check?username=s
    // user의 username이 동일한지 확인해줄래? -응답(json)
    @GetMapping("/api/user/username/same-check") // 예외적으로 동사 허용
    ...
    // usename(x), password(o),email(o)
    // password=1234&email=ssar@nate.com(x-www-form-urlecoded)
    // { "password" : "1234", "email" : "ssar@nate.com" } (application/json)
    // json을 받을것이기 때문에 Spring이 데이터 받을 때 파싱 전략을 변경해줘야 한다!!
    // Put 요청은 http body가 있다. http header의 content-type mime타입을 알려줘야 한다.
    // 유저수정 - 로그인O
